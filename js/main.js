...
if (!reducedMotion) { let scene; try { scene = new WebGLScene(); } catch { scene = new SceneController(); } const sections = stateIds.map(id => document.getElementById(id)).filter(Boolean); const observer = new IntersectionObserver(entries => entries.forEach(entry => { if (entry.isIntersecting) scene.setState(entry.target.id); }), { rootMargin: '-34% 0px -48% 0px', threshold: 0 }); sections.forEach(section => observer.observe(section)); }
